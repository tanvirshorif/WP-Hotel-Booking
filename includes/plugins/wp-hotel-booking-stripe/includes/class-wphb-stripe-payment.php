...
/**
 * WP Hotel Booking Stripe payment class.
 *
 * @class       WPHB_Payment_Gateway_Stripe
 * @version     2.0
 * @package     WP_Hotel_Booking_Stripe/Classes
 * @category    Class
...
if ( ! class_exists( 'WPHB_Payment_Gateway_Stripe' ) ) {
	 ...
	 * @since 2.0
	 */
	class WPHB_Payment_Gateway_Stripe extends WPHB_Abstract_Payment_Gateway {
		...
		/**
		 * WPHB_Payment_Gateway_Stripe constructor.
		 *
		 * @since 2.0
		 */
		public function __construct() {
			...
			$this->_title       = __( 'Stripe', 'wphb-stripe-payment' );
			$this->_description = __( 'Pay with Stripe', 'wphb-stripe-payment' );
			$this->_settings    = WPHB_Settings::instance()->get( 'stripe' );
		...
		/**
		 * Display when selected Stripe in checkout page.
		 *
		 * @since 2.0
		 */
		public function form() {
			echo __( 'Pay with Credit card', 'wphb-stripe-payment' );
		}
		...
		public function stripe_request( $request, $api = 'charges' ) {
			...
			if ( is_wp_error( $response ) ) {
				return new WP_Error( 'stripe_error', __( 'There was a problem connecting to the payment gateway.', 'wphb-stripe-payment' ) );
			}
			if ( empty( $response['body'] ) ) {
				return new WP_Error( 'stripe_error', __( 'Empty response.', 'wphb-stripe-payment' ) );
			}
			...
			if ( ! empty( $parsed_response->error ) ) {
				...
				return new WP_Error( 'stripe_error', __( 'Invalid response.', 'wphb-stripe-payment' ) );
			}
		...
		public function process_checkout( $booking_id = null ) {
			...
			if ( is_wp_error( $response ) ) {
				$return = array(
					'result'  => 'error',
					'message' => sprintf( __( '%s. Please try again', 'wphb-stripe-payment' ), $response->get_error_message() )
				);
			} else {
				if ( $response->id ) {
					...
					$return = array(
						'result'  => 'error',
						'message' => __( 'Please try again', 'wphb-stripe-payment' )
					);
				}
			}
