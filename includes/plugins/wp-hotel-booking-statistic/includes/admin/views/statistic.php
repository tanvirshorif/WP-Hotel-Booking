...
<ul class="tp_hotel_booking subsubsub">
	...
	<?php foreach ( $report_tabs as $key => $tab ): ?>
		...
		<?php $html[] =
			'<li>
				<a ' . ( $tab['id'] === $currenttab ? 'class="current" ' : '' ) . 'href="' . admin_url( 'admin.php?page=wphb-statistic&tab=' . $tab['id'] ) . '" >' . sprintf( '%s', $tab['title'] ) . '</a>
			</li>';
		?>
	<?php endforeach; ?>
...
<div id="tp-hotel-booking-report" class="postbox">
	...
	<div id="poststuff">
		<h3>
			<!-- <a href="<?php //echo admin_url( 'admin.php?page=tp_hotel_booking_report&action=export' ) ?>" class="export_csv"><?php //_e( 'Export CSV', 'wphb-statistic' ) ?></a> -->
			<!--export-->
			<form id="tp-hotel-booking-export" method="POST">
				...
				<input type="hidden" name="report_in_timestamp"
				       value="<?php echo isset( $_REQUEST['report_in_timestamp'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_in_timestamp'] ) ) : '' ?>">
				...
				<input type="hidden" name="report_out_timestamp"
				       value="<?php echo isset( $_REQUEST['report_out_timestamp'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_out_timestamp'] ) ) : '' ?>">
			...
			<ul>
				<?php foreach ( $date as $key => $d ): ?>
					<li <?php echo sprintf( '%s', $d['id'] === $currentRang ? 'class="active"' : '' ) ?>>
						<a href="<?php echo admin_url( 'admin.php?page=wphb-statistic&tab=' . $currenttab . '&range=' . $d['id'] ) ?>">
				...
				<li>
					<form method="GET">
						...
						<input type="text" id="tp-hotel-report-checkin" name="report_in"
						       value="<?php echo isset( $_REQUEST['report_in'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_in'] ) ) : ''; ?>"/>
						<input type="hidden" name="report_in_timestamp"
						       value="<?php echo isset( $_REQUEST['report_in_timestamp'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_in_timestamp'] ) ) : ''; ?>"/>
						<input type="text" id="tp-hotel-report-checkout" name="report_out"
						       value="<?php echo isset( $_REQUEST['report_out'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_out'] ) ) : ''; ?>"/>
						<input type="hidden" name="report_out_timestamp"
						       value="<?php echo isset( $_REQUEST['report_out_timestamp'] ) ? esc_attr( sanitize_text_field( $_REQUEST['report_out_timestamp'] ) ) : ''; ?>"/>
